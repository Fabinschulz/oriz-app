...
void main() async {
  ...
  var materialApp = MaterialApp(
    ...
    routes: setupAppRoutes(controller),
  );

  runApp(materialApp);
}

Map<String, WidgetBuilder> setupAppRoutes(TransactionController controller) {
  return {
    APP_ROUTES.HOME: (ctx) => HomeScreen(controller: controller),
    APP_ROUTES.NEW_TRANSACTIONS: (ctx) =>
        NewTransactionScreen(controller: controller),
  };
}
